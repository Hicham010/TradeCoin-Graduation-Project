...
function AddInformationHandler() {
  ...
  const title = "Grant Information Handler Role";

  async function addInformationHandler() {
    ...
    if (typeof window.ethereum !== "undefined") {
      ...
      let transaction;
      try {
        transaction = await contract.addInformationHandler(addressForRole);
  ...
  return (
    <Card
      title="Grant Information Handler Role"
      func={addInformationHandler}
      inputFields={fields}
      loading={loading}
    />
  );
...
export default AddInformationHandler;
